...
cfg_get() {
    ...
    case "$key" in
        ...
        bucket_name)
            ...
            ;;
        deployment_name)
            value=$(grep -E '^deployment_name:' "$CONFIG_FILE" 2>/dev/null | awk '{print $2}' | tr -d '"' || true)
            ;;
        environment)
            value=$(grep -E '^environment:' "$CONFIG_FILE" 2>/dev/null | awk '{print $2}' | tr -d '"' || true)
            ;;
...
# --- Deployment Identity ---
echo -e "${BOLD}--- Deployment Identity ---${NC}"
echo "  Every AWS resource is named {deployment_name}-{resource}-{environment},"
echo "  and the allocator scopes its client Terraform state to the S3 prefix"
echo "  {deployment_name}/{environment}/. These must match the -var values you"
echo "  pass to Terraform (the deploy workflow does this for you)."
DEFAULT_DEPLOYMENT_NAME="${LABLINK_DEPLOYMENT_NAME:-$(cfg_get deployment_name "my-lablink")}"
ask CFG_DEPLOYMENT_NAME "Deployment name (3-32 chars, lowercase kebab-case)" "$DEFAULT_DEPLOYMENT_NAME"

DEFAULT_ENVIRONMENT="${LABLINK_ENVIRONMENT:-$(cfg_get environment "prod")}"
ask CFG_ENVIRONMENT "Environment (dev, test, ci-test, prod)" "$DEFAULT_ENVIRONMENT"
...
deployment_name: "${CFG_DEPLOYMENT_NAME}"
